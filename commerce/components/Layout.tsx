...
const Layout = ({title, children}:any) => {
  ...
  const { theme, setTheme } = useTheme()
    ...
    return(
        <>
          <Head>
                <title>{title ? title + ' - Testing Clothes':'Testing Clothes'}</title>
         ...
         <div className=" flex min-h-screen  flex-col justify-between">
            ...
            <header>
                <nav className="flex h-12 justify-between shadow-md items-center">
                    <Link href="/">
                        <p className="text-lg ml-3 pl-2 font-bold">Testing Store</p>
                    </Link>
          ...
          <span className='font-semibold'>Copyright </span> 
           <p className='px-1'> &copy; </p>2025 Testing Store
          </div>
          <div className='flex justify-between px-2'>
            <Link className='flex px-1' href='https://www.instagram.com/'><BsInstagram/></Link>
            <Link className='flex px-3' href='https://api.whatsapp.com/send?phone=082122548332?text=testing'><BsWhatsapp/></Link>
          </div>
      ,
        </footer>
        </div>
